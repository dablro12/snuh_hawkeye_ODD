...
for gender_key in ['A', 'M', 'FM']:
    print(f"\nGender {gender_key} 전처리 중...")
    
    # Train 데이터 전처리
    train_df_gender = gender_train_data[gender_key]
    test_df_gender = gender_test_data[gender_key]
    
    if len(train_df_gender) == 0 or len(test_df_gender) == 0:
        print(f"  경고: Gender {gender_key} 데이터가 비어있습니다. 건너뜁니다.")
        continue
    
    X_train_gender = train_df_gender.drop(columns=['ODD_CP'])
    y_train_gender = train_df_gender['ODD_CP']
    ...
    # Test 데이터 전처리
    X_test_gender = test_df_gender.drop(columns=['ODD_CP'])
    y_test_gender = test_df_gender['ODD_CP']
    ...
    # 저장
    gender_X_train[gender_key] = X_train_gender
    gender_y_train[gender_key] = y_train_gender
    gender_X_test[gender_key] = X_test_gender
    gender_y_test[gender_key] = y_test_gender
    
    print(f"  ✅ Gender {gender_key} 전처리 완료")
...
for exp_idx, (feature_set, gender, feature_file, exp_name) in enumerate(EXPERIMENTS, 1):
    print(f"\n\n{'#'*80}")
    ...
    try:
        # 1. Gender별 전처리된 데이터 가져오기
        if gender not in gender_X_train:
            print(f"  경고: Gender {gender} 데이터가 없습니다. 실험을 건너뜁니다.")
            continue
        
        X_train_filtered = gender_X_train[gender]
        y_train_filtered = gender_y_train[gender]
        X_test_filtered = gender_X_test[gender]
        y_test_filtered = gender_y_test[gender]
        
        print(f"Gender 필터링 완료 ({gender}):")
        ...
        # 2. Top7 features 추출
        top7_df = pd.read_csv(feature_file)
        top7_features = top7_df['feature']
        selected_features = extract_features_with_onehot(top7_features, X_train_filtered.columns)
        
        print(f"\n{'='*80}")
        print(f"Top 7 Features (원본)")
        print(f"{'='*80}")
        print(top7_features.tolist())
        
        print(f"\n{'='*80}")
        print(f"선택된 Features (원핫인코딩 포함, 총 {len(selected_features)}개)")
        print(f"{'='*80}")
        print(selected_features)
        
        # 선택된 features만 사용
        X_train_top7 = X_train_filtered[selected_features]
        X_test_top7 = X_test_filtered[selected_features]
        
        print(f"\n✅ X_train_top7 shape: {X_train_top7.shape}")
        print(f"✅ X_test_top7 shape: {X_test_top7.shape}")
        print(f"✅ y_train shape: {y_train_filtered.shape}")
        print(f"✅ y_test shape: {y_test_filtered.shape}")
        
        # 3. 모델 학습 (LightGBM만 사용)
        ...
        multi_model_trainer.fit(
            X_train_top7, y_train_filtered, 
            X_test_top7, y_test=y_test_filtered
        )
        ...
        save_dict = {
            ...
            'top7_features': top7_features.tolist(),
            'test_inputs': X_test_top7,
        ...
        # Feature importance 저장 (LightGBM)
        ...
        print(f"  Feature Set: {feature_set}, Gender: {gender}")
        print(f"  Train 크기: {len(X_train_top7)}, Test 크기: {len(X_test_top7)}")
        print(f"  Train 클래스 분포: {y_train_filtered.value_counts().to_dict()}")
        print(f"  Test 클래스 분포: {y_test_filtered.value_counts().to_dict()}")
